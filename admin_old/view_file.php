...
<tbody>
    ...
    <tr>
      ...
      $query = mysqli_query($conn,"SELECT DISTINCT files.id,files.name,description,files.user_id,folder_id,file_type,is_public,date_updated,size,status,folders.name as fname,admin_login.name as admname,folders.id FROM files
      INNER JOIN folders ON folders.id=folder_id
      INNER JOIN admin_login ON admin_login.id=files.user_id
      
        group by name DESC") or die (mysqli_error($con));
      while($file=mysqli_fetch_array($query)){
         $id =  $file['id'];
         $name =  $file['name'];
         $size =  $file['size'];
         $email =  $file['status'];
         $uploads =  $file['admname'];
         $time =  $file['date_updated'];
